...
if ($result2->num_rows > 0 ): 
  ...
  //    echo $email;
              $subject = "Rappelez à les étudiants qui n'a pas encore évalué le matériel".' '.$id_mat[2];
              $message = "Bonjour chers étudiants, Nous demandons à évaluées avant l'expiration du délai qui y est indiqué. Je vous rappelle que cette évaluation est obligatoire dans toutes les matières, en cas de réponses manquantes vous serez sujets à une pénalité. 
              Bien à vous";
...
<option value="matier_deja_evaluee.php" selected>matieres deja evaluees</option>
 ...
 <table  class="table table-bordered" id="data_table">
    <thead sclass="header round py-5 my-5">
        <tr style="background-color:rgb(27, 24, 24);">
          ...
          <th style="color: white; width: 20%; background-color:rgb(0, 0, 224);">Nom</th>
          <th style="color: white; width: 10%; background-color:rgb(0, 0, 224);">debut</th>
          <th style="color: white; width: 10%; background-color:rgb(0, 0, 224);">Fin</th>
          <th style="color: white; width: 10%; background-color:rgb(0, 0, 224);">filiere</th>
          <th style="color: white; width: 10%; background-color:rgb(0, 0, 224);">Email</th>
          <th style="color: white; width: 7%; background-color:rgb(0, 0, 224);">N.etudiant</th>
          <th style="color: white; width: 7%; background-color:rgb(0, 0, 224);text-align:center">Statistique</th>
          <th style="color: white; width: 10%; background-color:rgb(0, 0, 224);text-align:center">Anuller</th>
    ...
    <tbody> 
        ...
        $query = "SELECT * FROM matieres WHERE id in (SELECT id_mat from `evaluation` WHERE id  in (SELECT id_evalu from reponse)and fin > NOW() );";
        $result = mysqli_query($conn, $query);
     ...
     ?>
                ...
                <td data-title="email" name="email" style="text-align:center"><button type="submit" name="a" class="btn btn-primary" value="<?php echo $array[1]; ?>" >email</button></td>
                <td data-title="nb" style="text-align:center"><a href="../tese_export/NB_etudiant.php?id=<?php echo $array[0]; ?>" id="btn1"><?php echo $a[0]; ?> </a></td>
                <td style="text-align:center"><a href="../tese_export/ouvert.php?code=<?php echo $array[1]; ?>"class="btn btn-success">Statisique</a></td>
                <td class='select' style="text-align:center"><a href="delete.php?id=<?php echo $arr[0]; ?>"id="btn1" class="btn btn-danger">Annuler</a></td>
        <?php

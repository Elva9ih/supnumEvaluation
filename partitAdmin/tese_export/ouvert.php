...
<select class="form-select" onchange="this.form.submit()" aria-label="Default select example" name="group" id="s" style="display: inline;">
<?php
if(isset($_POST["group"])){
...
echo '<option value="group">group</option>';
 $s_g = "SELECT DISTINCT(`group`) FROM `description` where id_evalu in(select id from evaluation where id_mat in(select id from matieres where codematieres='$_matiere'))";
...
// if(isset($_POST["group"])){
//     if($_POST["group"] != "group"){
//     $_id_eval=mysqli_query($conn,"SELECT id FROM evaluation WHERE id_mat in(SELECT id FROM matieres WHERE codematieres='$_matiere') and id in (select id_evalu from reponse where id_etud in(select id from etudiants where `group`='".$_POST["group"]."'))") ;
// }}
